...
// cntrller func to make revision 
export const makeRevision = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (user.credits < 1) {
            return res.status(401).json({ message: 'Add more credit to make changes....!!!' });
        }

        if (!message || message.trim() == '') {
            return res.status(400).json({ message: 'Pls Enter a valid prompt' });
        }
        ...
        await prisma.conversation.create({
            data: {
                role: 'assistant',
                content: 'Now Making Changes to Your Website... Please Wait ..!! ',
                projectId
            }
        })
        ...
        if (!code) {
            ...
            data: {
                role: 'assistant',
                content: "Unable To generate code pls try again !!",
                projectId
            }
        })
        await prisma.user.update({
...
// controller function to roll back to prev versions 
...
export const rollbackToVersion  = async (req: Request, res: Response) => {
    ...
    try {
        ...
        await prisma.conversation.create({
            data:{
                role:'assistant',
                content:"I've rolled back the website to selected Version. You can preview it",
                projectId
            }
        })
...
//controller func to delete any project
